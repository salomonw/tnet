...
class TrafficAssignment(object):
    ...
    def calculate_traveltime(self, fcoeffs):
        for edge in self.graph.edges():
            ...
            if capacity < self.threshold:
                ...
                traveltime = self.get_edge_attribute(edge, 't_0') * sum([fcoeffs[i] * (flow/capacity)**(i) for i in range(len(fcoeffs))])
    ...
    def run(self, fcoeffs=[1,0,0,0,0.15,0], build_t0=False):
        ...
        for i in range(1, self.n_iter_tm):
            ...
            self.calculate_traveltime(fcoeffs=fcoeffs)
