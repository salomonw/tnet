...
def set_up():
	...
	return [G_data, g_data, g_k, fcoeff, tNet, c1, d1, fcoeffs_truth, to_solve, iterations, dir_out]
...
fcoeffs_list = []
parameters = set_up()
[G_data, g_data, g_0, fcoeffs_0, tNet, c1, d1, fcoeffs_truth, to_solve, iterations, dir_out] = parameters
...
#plt.figure()
#plot(x_axis, flowNormConstant[1:], label='$f(\\cdot) =$ BPR', marker='s', markevery=(0,20))
#plot(x_axis, flowNormGD[1:], label='GD', marker = 'o', markevery=(0,10))
#plot(x_axis, flowNormAlternating[1:], label='Alternating', marker=11, markevery=(0,25))
#plt.plot(x_axis, flowNormJOINT[1:], label='Joint', marker = 'o', markevery=(0,10))
#xlabel("Iteration,$j$")
#ylabel("Flow error, $F(\\mathbf{g}, \\boldsymbol{\\beta})$")
#legend(framealpha=1)
#grid(linestyle='--', linewidth=1)
#savefig(dir_out +'/graphs/'+tNet.netFileName[9:-8]+'_errorCost_single.png')
